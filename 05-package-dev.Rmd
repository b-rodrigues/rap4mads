...
Let's write a function that does some basic summary statistics on these files:
...
This seems to be working well (in a one of the next sections we will learn how to systematize these
...
It is time to start documenting our functions, and package. Documentation in R is not just
...
files help define the dependencies of the package. Let's start with the simplest thing we can
...
So what's important is that the comments start with `#'`, and then you can use keywords that start
with `@` to define sections. The example above shows basically everything you need to know to 
...
can always reach and use private function by using `:::`, as in `package:::my_private_function()`).
...
There are several document you need to change to properly document your package. Some are optional
...
best to keep them. There already there, and by having them there, even if there redundant with the
NAMESPACE, someone reading the source code will know immediately where the functions come from.
But if you want, you can remove that, it'll work.
...
keep it on github.com). What matters, is what you don't see. We are going to add some more lines
...
- Depends: we can define hard dependencies here. Because I'm using the base pipe `|>` in my code, my package needs at least R version 4.1.
- Imports: these is where we list packages that our package needs to run. If these packages are not available, they will be installed when users install our package.
...
They are written in the R-Markdown format, which we will learn about in chapter 8.
