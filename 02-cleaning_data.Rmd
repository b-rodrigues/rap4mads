...
First install the `{rio}` package, then download the following datasets:
...
To know which packages are used by `{rio}`, you can take a look at this [vignette](https://cran.r-project.org/web/packages/rio/vignettes/rio.html).

If you need to import very large datasets (potentially several GBs), you might want to look at packages
like `{vroom}` (see the benchmark were a 1.5G csv file gets read in seconds by
`{vroom}` [here](https://vroom.r-lib.org/articles/benchmarks.html#reading-delimited-files). For even larger
files, take a look at `{arrow}` [here](https://arrow.apache.org/docs/r/). This package is able to efficiently read
very large files (`csv`, `json`, `parquet` and `feather` formats)

## A little aside on pipes

Since R version 4.1, a forward pipe  `|>` is included. It allows to do this:
...
Before R version 4.1, there was already a forward pipe, introduced with the `{magrittr}` package (and automatically loaded
...
Both expressions above are equivalent to `sqrt(4)`. You will see why this is useful very soon. For now, just know this.
...
really focus on sections 4.3, 4.4, 4.5.3, 4.5.4, 4.7 and 4.8.
