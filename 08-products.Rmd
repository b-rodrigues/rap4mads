...
be very annoying, very quickly. So the other strategy that we will use is to provide some visual 
...
from `{dplyr}` change the way they work depending on the type of the object their working with.
...
Let's now build a basic version of the app, without any fancy visual cues nor caching. Here is
the global file:
...
but even better than waiting is no waiting at all. For this, we are going to enable caching
...
will take time, as before. But if we perform the same computations again, then the results will show
instantly:
...
and show you the cache. There are three `.Rds` objects, these are the final data frames that
...
free credit to test things out. Vultr is quite generous as they give you up to 300USD. This
is more than enough to get a basic VPS with Ubuntu on it. You can then try to install everything
needed to deploy Shiny apps from your VPS. You could follow 
[this guide](https://www.marinedatascience.co/blog/2019/04/28/run-shiny-server-on-your-own-digitalocean-droplet-part-1/) 
to deploy from DigitalOcean, which should generalize well to other services like Vultr.
Doing this will teach you a lot, and I would highly recommend you do it.
