...
tar_option_set(packages = c("myPackage", "dplyr", "ggplot2"))
...
The first line simply loads the `{targets}` package, the second line sets the packages required for the pipeline
to actually run. This is equivalent to having a series of calls to `library()` in a normal script.
Then comes a list. Inside this list is where we will define the targets, or the (intemediary) outputs of
the pipeline. We defined `unemp_data` as being the output of the function `get_data()`... but where does this
function come from? Well, we need to create another script called `functions.R` where we will define every
function the we need for this pipeline. Let’s create an empty script and put the following lines in it:
...
This function is as simple as it gets; it doesn’t take any arguments and returns the data that we defined in our package.
...
You can now run the pipeline using `tar_make()`. This will not create any output, but this way you can at least test that
it’s running. Also, remember `tar_load()` and `tar_read()` from the walkthrough? First try to run `tar_read(unemp_data)`, 
this should print the data in your console. You can then run `tar_load(unemp_data)`, which this time loads the data in your 
global environment. You can now access it interactively. This is quite useful if you need to inspect intermediary outputs.

Let’s now add intermediary outputs; we need data for Luxebmourg, data for the cantons and data for some communes:
...
At this stage it might be interesting to take a look at the network. Call `tar_visnetwork()` and take a look at 
the pipeline:
...
make_plot <- function(data){
  ggplot(data) + 
    geom_line(aes(y = active_population, x = year, group = place_name, colour = place_name)) + 
...
Let’s now take a look at the pipeline again with `tar_visnetwork()`, and then run the pipeline using `tar_make()`. If everything
went well, we can now take a look at our plots using `tar_read(luxembourg_plot)`.

Finally, let’s write these plots to disk. The way to save a `ggplot` to disk is to use the `ggsave()` function. But targets have
to return something, so [side-effects](https://books.ropensci.org/targets/targets.html#side-effects) like writing to disk
must be handled in a specific way. What we’re going to do is write a wrapper around `ggsave()` that will take the path where
the plot should be save to disk, save the plot to the specified path, and then return the plot. This way, we have a function
that does not only have a side-effect. Let’s go back to `functions.R` and add the following lines:
...
list(
 ...
 tar_target(
    ...
    save_plot("fig/luxembourg.png", luxembourg_plot),
    format = "file"
  ),
...
this files for changes as well, and reruns the pipeline if a change is detected. Run the pipeline now with `tar_make()` and watch the plots
...
make_plot <- function(data){
  ggplot(data) + 
    ...
    theme(legend.position = "bottom",
          legend.title = element_blank()) +
    labs(title = paste0("Unemployement for ", paste(data$place_name, collapse = ", "))
}
```
...
Every targets that gets made by `make_plot()` must be recomputed, and every targets that depends on the intemediary outputs of 
`make_plot()` as well. Reloading the data is not necessary, since the edits on `make_plot()` do not affect these targets. This is
an incredible cognitive load that is taken off the data scientists; no need to keep track of outputs that are outdated, and no need
to re-run everything either, saving lots of time and processing power.

You now know the basics of setting up a reproducible (well, almost, as you’ll see) analytical pipeline. Let’s now move
to running someone else’s pipeline.
...
author of `{targets}`. To start clone this repository, and open an R session in the root of the folder.

Then call `renv::restore()`. You should see this:
...
in a new library, separate from the default library you’ve been using:
...
Once again, answer `y`. A list of packages to install will appear. For the last time press `y` to install 
all the packages. As you can see from the screenshot below, because the packages used for this pipeline
...
Before running the pipeline, load the `{tidyr}` package and then call `sessionInfo()`:
...
And take note of the `{tidyr}` version:

Now, open a completely fresh R session, and do the same. You should see that `{tidyr}`’s version is 
different:

This is because the version of `{tidyr}` needed for the pipeline is older, and gets loaded correctly 
for the pipeline.
